...
output_csv = "predicciones_teacher_forcing_parcial_CLIENTES.csv"
model_save_path = "modelo_teacher_forcing_parcial.pt"
...
class AutoregressiveLSTM(nn.Module):
    ...
    def forward(self, x_hist, future_labels=None, future_len=None, forcing_prob=0.7):
        ...
        for t in range(future_len):
            ...
            if future_labels is not None and np.random.rand() < forcing_prob:
...
print("🧠 Comenzando entrenamiento con Teacher Forcing parcial (50%)...")
...
for epoch in range(1, n_epochs + 1):
    ...
    for x_hist, y_true, _ in dataloader:
        ...
        logits = model(x_hist, future_labels=y_true.unsqueeze(0), future_len=len(y_true), forcing_prob=forcing_prob)
        loss = criterion(logits, y_true)
        loss.backward()
        optimizer.step()
        total_loss += loss.item()

    print(f"Epoch {epoch:2d} - Loss: {total_loss:.4f} - Forcing Prob: {forcing_prob:.2f}")
