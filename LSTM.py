...
country_csv = os.path.join(base_dir, 'client_util_sale_with_state.csv')
...
# Construye DataFrame con probabilidades y clase entera predicha
preds_df = pd.DataFrame({
    'sale_id':             sale_ids_val,
    'p_CORRECTO':          probs[:, 0],
    'p_PENDIENTE_DE_PAGO': probs[:, 1],
    'p_IMPAGO':            probs[:, 2],
    'pred_state':          probs.argmax(axis=1)  # estado como entero
})

# Guarda CSV
output_path = os.path.join(preds_dir, 'predicciones_CLIENTES.csv')
